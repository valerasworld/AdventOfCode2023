...
class PipeMaze2 {
    ...
    func amountOfInnerPoins(input: String) -> Int {
        ...
        // MARK: - Find "S" and add visited points into Array
        ...
        for (rowIndex, row) in grid.enumerated() {
            for (columnIndex, char) in row.enumerated() {
                if char == "S" {
                    ...
                    func describeSDirections(direction: [Int]) -> String {
                        var directionString: String = ""
                        if direction == [rowIndex + 1, columnIndex] {
                            directionString = "down"
                        }
                        if direction == [rowIndex, columnIndex - 1] {
                    ...
                    var directionsArrayStrings: [String] = []
                    
                    // Find the first possible Pipe for S:
                    for sDirection in sDirections {
                        guard sDirection[0] >= 0 && sDirection[0] < grid.count && sDirection[1] >= 0 && sDirection[1] < row.count else { continue }
                        ...
                        if sDirection == [rowIndex + 1, columnIndex] && (currentChar == vPipe || currentChar == bigLPipe || currentChar == bigJPipe) {
                            ...
                            directionsArrayStrings.append(describeSDirections(direction: sDirection))
                        }
                    ...
                    visitedPoints.append(sCoordinates)
                    previousCoordinates = sCoordinates
                    
                    
                    // Get "S" character
                    
                    sChar = getSCharacter(directions: directionsArrayStrings)
                    func getSCharacter(directions: [String]) -> Character {
                        var sChar: Character = " "
                        
                        if directions.contains("down") && directions.contains("left") {
                            sChar = "7"
                        }
                        if directions.contains("down") && directions.contains("up") {
                            sChar = "|"
                        }
                        if directions.contains("down") && directions.contains("right") {
                            sChar = "F"
                        }
                        ...
                        if directions.contains("up") && directions.contains("right") {
                            sChar = "L"
                        }
                    ...
                    while currentChar != "S" {
                        ...
                        visitedPoints.append(currentCoordinates)
        ...
        grid[sCoordinates[0]][sCoordinates[1]] = sChar
        
        
        for (rowIndex, row) in grid.enumerated() {
            var crossingPOINTS: Int = 0
            for (columnIndex, char) in row.enumerated() {
                print("Row: \(rowIndex), Column: \(columnIndex)")
                guard columnIndex != (row.count - 1) else { continue }
                let currentPoint: [Int] = [rowIndex, columnIndex]
                
                if !visitedPoints.contains(currentPoint) && crossingPOINTS == 0 {
                    for nextColumnIndex in (columnIndex + 1)...(row.count - 1) {
                        let nextPoint: [Int] = [rowIndex, nextColumnIndex]
                        let nextCharacter = grid[rowIndex][nextColumnIndex]
                        if visitedPoints.contains(nextPoint) {
                            if nextCharacter == "|" || nextCharacter == "L" || nextCharacter == "J" {
                                crossingPOINTS += 1
                            }
                        }
                    }
                    
                    if crossingPOINTS % 2 == 0 {
                        continue
                    } else {
                        count += 1
                    }
                ...
                if visitedPoints.contains(currentPoint) && (char == "|" || char == "L" || char == "J") {
                    crossingPOINTS -= 1
                }
...
//PipeMaze2().amountOfInnerPoins(input: PipeMaze2().shortInput)   // Right Answer: 4
//PipeMaze2().amountOfInnerPoins(input: PipeMaze2().shortInput2)  // Right Answer: 8
//PipeMaze2().amountOfInnerPoins(input: PipeMaze2().shortInput3)  // Right Answer: 10

//PipeMaze2().amountOfInnerPoins(input: PipeMaze2().longInput)    // Right Answer: 349
