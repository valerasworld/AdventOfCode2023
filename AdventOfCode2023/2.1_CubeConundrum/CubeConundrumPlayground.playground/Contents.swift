...
class CubeConundrum {
    ...
    func sumOfPossibleGames(input: String) -> Int {
        ...
        // bounds
        var maxAmountOfCubes: [String: Int] = ["red": 12, "green": 13, "blue": 14]
        
        // check if the amount of cubes is not out of bounds for EVERY color and add the gameNum to sum, otherwise do not add the gameNum to sum
        for (gameNum, attempts) in allGames {
            ...
            for (color, _) in attempts {
                // case when the amount is higher than maxAmount
                if (attempts[color] ?? 0) > maxAmountOfCubes[color]!  {
                    colorsToPass += 0
                } else {
                    // case when the amount is less or equal to maxAmount
                    colorsToPass -= 1
                }
            }
            
            // case when each color's value is in bounds
            if colorsToPass == 0 {
                sum += gameNum
            } else {
                // case when at least one color's value is out of bounds
                sum += 0
            }
        }
...
CubeConundrum().sumOfPossibleGames(input: CubeConundrum().inputShort) // Right Answer is 8
CubeConundrum().sumOfPossibleGames(input: CubeConundrum().inputLong) // Right Answer is 2207
