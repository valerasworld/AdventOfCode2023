...
class CubeConundrum2 {
    
    // Test Cases:
    var inputShort: String {
        let input =
        """
    ...
    func sumOfPossibleGames(input: String) -> Int {
        var sum: Int = 0
        var allGames: [Int: [String: Int]] = [:] // [gameNum: [color: maxAmount]]
        
        // separate String lines to [gameString]
        let separatedLines: [String] = input.components(separatedBy: "\n")
        
        // separate each gameString to gameNum and attemptsString
        for line in separatedLines {
                        
            var wholeLine = line
            let gameNumberAndAllAttemptsStrings: [String] = wholeLine.components(separatedBy: ": ")
            
            // get gameNumber
            // Example: "Game 5" -> 5
            var gameNumString = gameNumberAndAllAttemptsStrings[0]
            gameNumString = gameNumString.filter({ $0.isNumber })
            var gameNum = Int(gameNumString) ?? 0 // ! Number of the game
            
            // all attempts for current game
            var allAttemptsForCurrentGame: [String: [Int]] = [:]
            
            // get allAttempts string
            // Example: "6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green" ->
                // -> ["6 red, 1 blue, 3 green", "2 blue, 1 red, 2 green"]
            var gameTriesString: [String] = gameNumberAndAllAttemptsStrings[1].components(separatedBy: "; ")
            
            // separate allAttempts to each color bunches
            // Example: "6 red, 1 blue, 3 green" -> ["6 red", "1 blue", "3 green"]
            for bunchOfCubes in gameTriesString {
                var bunchString: [String] = bunchOfCubes.components(separatedBy: ", ")
                
                var attempt: [String: Int] = [:]
                
                // separate each bunch to colors and values
                for valueAndColorString in bunchString {
                    // Example: "6 red" -> 6
                    let value: Int = Int(valueAndColorString.filter({ $0.isNumber })) ?? 0
                    // Example: "6 red" -> "red"
                    let color: String = valueAndColorString.filter({ $0.isLetter })
                    
                    // Example: ["red": 6, ...]
                    attempt[color] = value
                }
                
                // add attemp dictionary data to allAttempts of the current game dictionary
                // Example: ["blue": 2, "red": 1, "green": 2]
                for (color, value) in attempt {
                    // if the color already exists in allAttemptsForCurrentGame, we can append the value to the array and assign the new array back to the color in dictionary
                    if var existingValues = allAttemptsForCurrentGame[color] {
                        existingValues.append(value)
                        allAttemptsForCurrentGame[color] = existingValues
                    } else {
                        // in case there is no any value for the color, create a new array of values for this color and add a value to the array
                        allAttemptsForCurrentGame[color] = [value]
                    }
                }
            }
            // for this problem we need to find the minimal necessary amount of cubes of each color so the game could be possible -> it means that highest attemp for each color will be the same
            var highestAttemptForEachColor: [String: Int] = [:]
            
            // find the maxValue from the array of values for each color
            // Example: "blue": [3, 1, 7] -> "blue": 7
            for (color, arrayOfValues) in allAttemptsForCurrentGame {
                // find the highestAttempt for each color
                if let maxValue = arrayOfValues.max() {
                    highestAttemptForEachColor[color] = maxValue
                }
            }
            
            // assign dictionary of the highestAttempts for each color to the current gameNumber
            allGames[gameNum] = highestAttemptForEachColor
        }
        
        // we need to find the power of the each game that is produce of all colors' values and find the sum of all powers
        for (_, valuesForEachColor) in allGames {
            // we're sure that every color's value > 0, therefore we can initially assign powerOfSet to 1 to use `*=` operator safely
            var powerOfSet: Int = 1
            
            for (_, value) in valuesForEachColor {
                powerOfSet *= value
            }
...
CubeConundrum2().sumOfPossibleGames(input: CubeConundrum2().inputShort) // Right Answer is 2286
CubeConundrum2().sumOfPossibleGames(input: CubeConundrum2().inputLong) // Right Answer is 62241
