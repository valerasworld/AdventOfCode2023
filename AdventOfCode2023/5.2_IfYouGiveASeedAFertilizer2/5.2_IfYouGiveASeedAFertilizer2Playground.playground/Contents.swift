...
class IfYouGiveASeedAFertilizer2 {
    ...
    func findMinLocationForActualSeeds(input: String) -> Int {
        ...
        for (charIndex, char) in seedsString.enumerated() {
            ...
            if char.isNumber && (charIndex == seedsString.count - 1) {
                ...
                seedsData.append(Int(numberString) ?? 0)
                continue
            }
        ...
        // MARK: - Actual Seeds -> [[[Int]]]
        var actualSeeds: [Int: Int] = [:]
        
        for (index, number) in seedsData.enumerated() {
            if index % 2 == 0 {
                actualSeeds[number] = seedsData[index + 1]
            } else {
                continue
            }
        }
...
//            print("Ranges Array: \(rangesArray)")
            
            allMaps.append(rangesArray)
...
//                print("🌱 Seed is: \(seed)")
                ...
                for level in allMaps {
                    for range in level {
                        let sourceRange: Int = range[1]
                        let destinationRange: Int = range[0]
                        let rangeLength: Int = range[2]
                        
                        let rangeBounds = sourceRange + rangeLength - 1
                        
                        // case if current value is out of bounds
                        if currentValue < sourceRange || currentValue > rangeBounds {
                            continue
                        } else if (currentValue >= sourceRange) && (currentValue <= rangeBounds) {
                            // case when it is in bounds -> we don't need to check other ranges -> can `break`
                            currentValue = currentValue - sourceRange + destinationRange
                            break
                        }
                    }
                    //
                ...
                // check if we already have a minLocation value after previous levels
