...
class Trebuchet2 {
    ...
    func sumOfActual(input: String) -> Int {
        let separatedLines = input.components(separatedBy: "\n")
        var sum: Int = 0
        ...
        for line in separatedLines {
            ...
            newLine = newLine.replacingOccurrences(of: "one", with: "o1e")
            newLine = newLine.replacingOccurrences(of: "two", with: "t2o")
            newLine = newLine.replacingOccurrences(of: "three", with: "t3e")
            newLine = newLine.replacingOccurrences(of: "four", with: "f4r")
            newLine = newLine.replacingOccurrences(of: "five", with: "f5e")
            newLine = newLine.replacingOccurrences(of: "six", with: "s6x")
            newLine = newLine.replacingOccurrences(of: "seven", with: "s7n")
            newLine = newLine.replacingOccurrences(of: "eight", with: "e8t")
            newLine = newLine.replacingOccurrences(of: "nine", with: "n9e")

            newLine = newLine.filter({ $0.isNumber })
            
            // edge case - if it's the only number in the line:
            if newLine.count == 1 {
                var duplicate = newLine + newLine
                sum += Int(duplicate) ?? 0
            } else {
                // regular case - if it's multiple numbers in the line:
                let firstCharIndex = newLine.index(newLine.startIndex, offsetBy: 0)
                let lastCharIndex = newLine.index(newLine.startIndex, offsetBy: newLine.count - 1)
                var stringNumber = String(newLine[firstCharIndex]) + String(newLine[lastCharIndex])
...
Trebuchet2().sumOfActual(input: Trebuchet2().inputShort) // Right Answer is 281
Trebuchet2().sumOfActual(input: Trebuchet2().inputLong) // Right Answer is 55701
